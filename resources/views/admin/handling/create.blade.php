...
@section('konten')
    ...
    <div id="main">
      <div class="row">
		...
		<form action="{{ url('/'.Request::segment(1).'/'.Crypt::encrypt($complaint->id)) }}" method="POST" enctype="multipart/form-data" class="form-horizontal">
		{{ csrf_field() }}
        ...
        <div class="col s12">
          <div class="container">
			...
			<div style="bottom: 90px; right: 19px;" class="fixed-action-btn direction-top">
				...
				<a href="{{ url('/'.Request::segment(1).'/'.Crypt::encrypt($complaint->id)) }}" class="btn-floating btn-large waves-effect waves-light red darken-2"><i class="material-icons">arrow_back</i></a>
			</div>
          </div>
		</form>
