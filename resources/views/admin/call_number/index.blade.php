								...
								<div class="col s12 " style="overflow-x:auto;">
									...
									<thead>
										<tr style="background-color: gray;color:white;border: 1px solid #f4f4f4;">
											...
											<th>No. Panggilan PSC</th>
											<th>Keterangan</th>
											<th style="width: 25%">#aksi</th>
										</tr>
									</thead>
									<tbody>
										@foreach($call_number as $v)
										<tr>
											<td>{{ ($call_number ->currentpage()-1) * $call_number ->perpage() + $loop->index + 1 }}</td>
									...
									<div class="float-right">{{ $call_number->appends(Request::only('search'))->links() }}</div>
								</div>
							</div>
							</div>
							@endcan
