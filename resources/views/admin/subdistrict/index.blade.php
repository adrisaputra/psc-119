							...
							<div class="row">
								...
								<div class="col s12 " style="overflow-x:auto;">
									...
									<tbody>
										...
										<tr>
											<td>{{ ($subdistrict ->currentpage()-1) * $subdistrict ->perpage() + $loop->index + 1 }}</td>
											<td>{{ $v->name }}</td>
											<td>
												<a class="mb-12 btn waves-effect waves-light cyan darken-1 btn-small" href="{{ url('/village/'.Crypt::encrypt($v->id) ) }}">Kelurahan</a>
												@can('ubah-data')
									...
									<div class="float-right">{{ $subdistrict->appends(Request::only('search'))->links() }}</div>
								</div>
							</div>
							</div>
							@endcan
